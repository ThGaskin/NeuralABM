...
class OpinionDynamics_ABM:
    ...
    def __init__(self,
        ...
        self.adj_matrix = torch.from_numpy(nx.to_numpy_matrix(network)).float() if network is not None else None
        ...
        to_learn = {}
        idx = 0
        for pname, p in {'epsilon': epsilon, 'mu': mu, 'sigma': sigma, 'network': network}.items():
            if p is None:
                to_learn[pname] = idx
                idx += 1
        ...
        self.initial_opinions = torch.rand((self.N, 1), dtype=torch.float) if init_values is None else init_values
        self.current_opinions = self.initial_opinions.clone()

        # Collect the edge weight properties
        self.edge_weights = np.array(
            [torch.exp(-abs(self.current_opinions[j]-self.current_opinions[i])).detach().numpy()
             for i, j in self.network.edges()]).flatten() if self.network is not None else None
    ...
    def update_edge_weights(self):
        ...
        if self.network is not None:
            self.edge_weights = np.array(
                [torch.exp(-abs(self.current_opinions[j] - self.current_opinions[i])).detach().numpy()
                 for i, j in self.network.edges()]).flatten()

    def run_single(self, *, current_values, input_data: torch.tensor = None, sigma: float = None, requires_grad: bool = True):
        ...
        # Get the parameters
        adj_matrix = self.adj_matrix if 'network' not in self.to_learn.keys() \
            else torch.reshape(input_data[self.to_learn['network']:self.to_learn['network']+(self.N)**2],
                               (self.N, self.N))
        epsilon = self.epsilon if 'epsilon' not in self.to_learn.keys() \
            else input_data[self.to_learn['epsilon']]
        mu = self.mu if 'mu' not in self.to_learn.keys() \
        ...
        sigma = sigma if 'sigma' not in self.to_learn.keys() \
        ...
        norms = torch.maximum(torch.reshape(torch.matmul(torch.ceil(adj_matrix), kernel).diag(), (self.N, 1)),
                              torch.ones_like(current_values))

        # Perform the opinion update
        new_values = new_values + mu*torch.reshape(torch.matmul(adj_matrix, kernel * diffs).diag(), (self.N, 1))/norms
