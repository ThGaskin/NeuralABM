...
class CNLS_Solver:
    def __init__(self, *, t: torch.Tensor, z: torch.Tensor, parameters: dict, **__):
        ...
        :param parameters: dictionary of the cavity properties, containing:
            ...
            - Omega: bandwidth of the gain media (denoted by tau in original code)
    ...
    def energy(self, state: torch.Tensor) -> torch.Tensor:
        ...
        :param state: the beam state (u, v) in real space
        """
        u, v = state
    ...
    def solve(self, initial_conditions, *, method: str = 'implicit_adams') -> torch.Tensor:
        ...
        :param: method: solver method, passed to torchdiff.odeint. Default is the 'implicit Adams' solver
        ...
        def _cnls_rhs(_z, _state) -> torch.Tensor:
            """Right-hand side of the CNLS. This function is passed to the solver. The state must be passed as a
            flattened array.

            :param: _z: current _z value
            :param: _state: current state (u, v) of the Laser beam
            :return: the state of the Laser beam after one round trip
            """
        ...
        # Use the torchdiffeq.odeint function to solve the ODE
        _solution = odeint(
            _cnls_rhs,
            t=self.z,
...
class Laser_cavity:
    ...
    def calculate_transformation_matrix(self, alpha: torch.Tensor = None) -> torch.Tensor:
        """Calculates the transformation matrix, given by the product of the Jones matrices of the individual
        waveplate and polarizer transformations. This matrix is used to adjust the phase shift of the Laser beam.

        :param alpha: vector of angles
        :return: the transformation matrix
        """

        # Get the angles; default is the current angle configuration
    ...
    def set_state(self, state: torch.Tensor, *, set_init: bool = False) -> None:
        ...
        :param state: state to use as current state
        :param set_init: store this state as the new initial condition to which the Laser can be reset
        """
        self.state = state
        if set_init:
    ...
    def round_trip(self, *, update_self: bool = True, method: str = None) -> tuple:
        ...
        :param update_self: (optional) whether to update the state of the laser. True by default, but can be turned off
            for testing.
        :param method: ODE solver, passed to solver.solve. Default is 'implicit_adams'
        ...
        new_state = self.solver.solve(self.state, method=method)
