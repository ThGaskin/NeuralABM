...
Kuramoto = import_module_from_path(
    mod_path=up(up(__file__)), mod_str="Kuramoto"
)
...
class Kuramoto_NN:
    ...
    def __init__(
        ...
        self.loss_function = base.LOSS_FUNCTIONS[loss_function.get("name").lower()](loss_function.get("args", None),
        ...
        self._dset_predictions.attrs["dim_names"] = [
            "time",
            "i", "j"
        ]
    ...
    def epoch(self, *, training_data, batch_size: int):
        ...
        for dset in training_data:
            ...
            for batch_no, batch_idx in enumerate(batches[:-1]):
                ...
                loss = (
                        loss
                        + torch.trace(pred_adj_matrix)
                )

                loss = loss + torch.nn.MSELoss()(pred_adj_matrix, torch.transpose(pred_adj_matrix, 0, 1))

                loss.backward()
                self.neural_net.optimizer.step()
                self.neural_net.optimizer.zero_grad()
                self.current_loss = loss.clone().detach().numpy().item()
                self.current_frob_error = torch.nn.functional.mse_loss(self.true_network, torch.reshape(self.current_predictions,
...
if __name__ == "__main__":
    ...
    eigen_frequencies = torch.stack(list(nx.get_node_attributes(network, "eigen_frequency").values()))
