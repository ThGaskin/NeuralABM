...
def test_data_generation(tmpdir):
    ...
    for name, config in test_cfg.items():
      ...
      synthetic_data = SIR.get_SIR_data(data_cfg=config, h5group=h5group, write_init_state = False)
      ...
      if sigma == 0:
        assert (torch.round(torch.sum(synthetic_data, dim=1), decimals=4) == torch.tensor([1.0])).all()
      else:
